...
st.markdown("""
    ...
    /* Intelligence Center Tabs (กรอบเขียวทึบ) */
    .stTabs [data-baseweb="tab-list"] { gap: 10px; }
    .stTabs [data-baseweb="tab"] { 
        background-color: rgba(255, 255, 255, 0.05); 
        border-radius: 4px; padding: 10px 20px; color: #8b949e; 
    }
    ...
    .footer { position: fixed; left: 0; bottom: 0; width: 100%; background-color: rgba(13, 17, 23, 0.95); text-align: center; padding: 10px; border-top: 1px solid rgba(255, 255, 255, 0.1); font-size: 0.8rem; color: #8b949e; z-index: 999; }
...
with st.sidebar:
    ...
    # หุ้นเด่นวันนี้
    st.markdown('<div class="top-pick-container"><p class="top-pick-title">🌟 หุ้นเด่นวันนี้ (Real-time)</p><div class="top-pick-item"><span>PTT.BK</span><span style="color:#8b949e;">Active</span></div><div class="top-pick-item"><span>EA.BK</span><span style="color:#8b949e;">Active</span></div><div class="top-pick-item"><span>GULF.BK</span><span style="color:#8b949e;">Active</span></div><div class="top-pick-item"><span>SCB.BK</span><span style="color:#8b949e;">Active</span></div><div class="top-pick-item"><span>CPALL.BK</span><span style="color:#8b949e;">Active</span></div></div>', unsafe_allow_html=True)
        ...
        for idx, (sym, d) in enumerate(analysis.items()):
            with tabs[idx]:
                ...
                with st.expander("📟 View Terminal Risk Log", expanded=False):
                    st.markdown(f'<div class="log-terminal"><div class="log-entry">[{datetime.now().strftime("%H:%M:%S")}] > ANALYZING {sym}...</div><div class="log-entry">[{datetime.now().strftime("%H:%M:%S")}] > STATUS: COMPLETED.</div></div>', unsafe_allow_html=True)
