...
if terminal_mode == "🏛️ Thai Climate Risk Project":
    with st.sidebar:
        with st.expander("🔍 Thai Stock Selection", expanded=True):
            t1 = st.text_input("Stock 1", "PTT.BK", key="th_cli1")
            t2 = st.text_input("Stock 2", "GULF.BK", key="th_cli2")
            t3 = st.text_input("Stock 3", "", key="th_cli3")
    ...
    if analysis:
        ...
        for i, (symbol, d) in enumerate(analysis.items()):
            with tabs[i]:
                ...
                with st.expander("📟 Terminal Log", expanded=False): st.markdown(f'<div class="log-terminal"><div class="log-entry">[{datetime.now().strftime("%H:%M:%S")}] SYSTEM: Analytical data for {symbol} synchronized.</div></div>', unsafe_allow_html=True)
...
# ==========================================
    ...
    with st.sidebar:
        with st.expander("🔍 Thai Stock Selection", expanded=True):
            t1 = st.text_input("Stock 1", "CPALL.BK", key="th_tr1"); t2 = st.text_input("Stock 2", "AOT.BK", key="th_tr2"); t3 = st.text_input("Stock 3", "", key="th_tr3")
    ...
    if analysis:
        ...
        for i, (symbol, d) in enumerate(analysis.items()):
            with tabs[i]:
                st.markdown(f"### 🇹🇭 {symbol} - Market Trading Signal"); df = d['history'].to_frame(); df['MA_Short'] = df['Close'].rolling(window=ma_s).mean(); df['MA_Long'] = df['Close'].rolling(window=ma_l).mean()
                ...
                c1, c2, c3, c4 = st.columns(4); curr_p = d['price']; prev_p = df['Close'].iloc[-2]; p_change = ((curr_p - prev_p) / prev_p) * 100; rsi_val = df['RSI'].iloc[-1]; trend_label = "📈 BULLISH" if df['MA_Short'].iloc[-1] > df['MA_Long'].iloc[-1] else "📉 BEARISH"
                ...
                fig_t = go.Figure(); fig_t.add_trace(go.Scatter(x=df.index, y=df['Close'], name="Price", line=dict(color='#00ff88'))); fig_t.add_trace(go.Scatter(x=df.index, y=df['MA_Short'], name=f"MA{ma_s}", line=dict(color='#f1e05a', dash='dash'))); fig_t.add_trace(go.Scatter(x=df.index, y=df['MA_Long'], name=f"MA{ma_l}", line=dict(color='#da3633', dash='dot'))); fig_t.update_layout(height=450, template="plotly_dark", margin=dict(l=0,r=0,t=20,b=0)); st.plotly_chart(fig_t, use_container_width=True, key=f"th_trade_{symbol}")
                st.divider(); st.subheader(f"📊 Deep Dive: {symbol} Fundamentals"); p1, p2 = st.columns([1, 2]); ret_1y = ((curr_p / df['Close'].iloc[-252]) - 1) * 100
                ...
                if df['MA_Short'].iloc[-1] > df['MA_Long'].iloc[-1] and rsi_val < 70: st.success(f"✅ **Signal: BUY** - หุ้น {symbol} อยู่ในแนวโน้มขาขึ้นและราคายังมี Upside")
                elif df['MA_Short'].iloc[-1] < df['MA_Long'].iloc[-1] and rsi_val > 30: st.error(f"⚠️ **Signal: SELL / WAIT** - แนวโน้มหลักเป็นขาลง แนะนำให้รอการกลับตัว")
                else: st.warning("🔄 **Signal: NEUTRAL** - สัญญาณยังไม่ชัดเจน"); st.markdown('</div>', unsafe_allow_html=True)

# ==========================================
# PAGE 3: Global Technical Analysis (รักษาไว้ครบถ้วน 100%)
# ==========================================
    ...
    with st.sidebar:
        ...
        with st.expander("📈 Strategy Settings", expanded=True):
            g_ma_s = st.slider("Short-Term MA ", 5, 50, 20); g_ma_l = st.slider("Long-Term MA ", 50, 200, 50); g_rsi = st.slider("RSI Window ", 7, 30, 14)
    ...
    if analysis:
        ...
        for i, (symbol, d) in enumerate(analysis.items()):
            with tabs[i]:
                st.markdown(f"### 🌎 {symbol} - Global Market"); df = d['history'].to_frame(); df['MA_S'] = df['Close'].rolling(window=g_ma_s).mean(); df['MA_L'] = df['Close'].rolling(window=g_ma_l).mean()
                exp1 = df['Close'].ewm(span=12, adjust=False).mean(); exp2 = df['Close'].ewm(span=26, adjust=False).mean(); df['MACD'] = exp1 - exp2; df['Signal'] = df['MACD'].ewm(span=9, adjust=False).mean()
                ...
                c1.metric("Current Price", f"${curr_p:,.2f}", f"{p_change:+.2f}%"); c2.metric("Trend Status", trend_label); c3.metric("RSI Momentum", f"{rsi_val:.2f}"); c4.metric("MACD Signal", "BUY" if df['MACD'].iloc[-1] > df['Signal'].iloc[-1] else "SELL")
                fig_g = go.Figure(); fig_g.add_trace(go.Scatter(x=df.index, y=df['Close'], name="Price", line=dict(color='#00ff88'))); fig_g.add_trace(go.Scatter(x=df.index, y=df['MA_S'], name=f"MA{g_ma_s}", line=dict(color='#f1e05a', dash='dash'))); fig_g.add_trace(go.Scatter(x=df.index, y=df['MA_L'], name=f"MA{g_ma_l}", line=dict(color='#da3633', dash='dot'))); fig_g.update_layout(height=450, template="plotly_dark", margin=dict(l=0,r=0,t=20,b=0)); st.plotly_chart(fig_g, use_container_width=True, key=f"gl_trade_{symbol}")
