...
# --- 🛠️ FIXED: SAFE DATA ENGINE (No KeyError) ---
...
def fetch_terminal_data(ticker_list):
    ...
    for symbol in ticker_list:
        try:
            ...
            # 💡 แก้ไข: ใช้ค่าคงที่หรือคำนวณแบบปลอดภัย ป้องกัน KeyError 'Carbon'
            c_beta = 0.0
            try:
                # ดึงแค่หุ้นกับตลาดมาเทียบกันแบบง่ายๆ เพื่อความเสถียร
                mkt = yf.download("^SET.BK", period="1y", progress=False)['Close']
                if not mkt.empty:
                    df = pd.concat([hist, mkt], axis=1).pct_change().dropna()
                    df.columns = ['Stock', 'Market']
                    c_beta = df['Stock'].cov(df['Market']) / df['Market'].var()
            except: pass
