...
if current_tickers:
    ...
    if terminal_mode == "🏛️ Thai Climate Risk Project":
        ...
        if analysis_data:
            ...
            for i, (symbol, d) in enumerate(analysis_data.items()):
                with tabs[i]:
                    st.markdown(f"### 🇹🇭 {symbol} - Predict Signal"); df = d['history'].to_frame(); df['MA_S'] = df['Close'].rolling(window=ma_s).mean(); df['MA_L'] = df['Close'].rolling(window=ma_l).mean()
                    ...
                    st.plotly_chart(go.Figure([go.Scatter(x=df.index, y=df['Close'], name="Price", line=dict(color='#00ff88')), go.Scatter(x=df.index, y=df['MA_S'], name="MA Short"), go.Scatter(x=df.index, y=df['MA_L'], name="MA Long")]), use_container_width=True, key=f"th_trade_{symbol}")
                    ...
                    elif df['MA_S'].iloc[-1] < df['MA_L'].iloc[-1] and rsi_val > 35: st.error(f"⚠️ **Bearish Alert:** {symbol} อยู่ในทิศทางขาลง แนะนำให้รอการกลับตัว")
